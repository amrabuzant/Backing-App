...
public class RecipeDetailActivity extends AppCompatActivity implements RecipeDetailFragment.OnStepClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(savedInstanceState==null) {
            ...
            fragmentManager.beginTransaction().add(R.id.detailFragmentContainer,recipeDetailFragment).commit();

            if(isTwoPane){

                stepDetailFragment=new StepDetailFragment();
                fragmentManager=getSupportFragmentManager();
                fragmentManager.beginTransaction().replace(R.id.stepDetailFragmentContainer,stepDetailFragment).commit();
    ...
    public void onStepSelected(int position, ArrayList<Step> steps) {
        if(!isTwoPane){
            stepDetailFragment=new StepDetailFragment();
            fragmentManager=getSupportFragmentManager();
            fragmentManager.beginTransaction().replace(R.id.detailFragmentContainer,stepDetailFragment).commit();
            stepDetailFragment.setStep(steps.get(position));
        }
        else {
            stepDetailFragment=new StepDetailFragment();
            ...
            case android.R.id.home:
                if(isTwoPane){
                    ...
                    try {if(recipeDetailFragment.isVisible()){
                        ...
                        getSupportActionBar().setTitle(recipeName);
                        fragmentManager=getSupportFragmentManager();
                        fragmentManager.beginTransaction().replace(R.id.detailFragmentContainer,recipeDetailFragment).commit();
                        prev.setVisibility(View.INVISIBLE);
                        next.setVisibility(View.INVISIBLE);
